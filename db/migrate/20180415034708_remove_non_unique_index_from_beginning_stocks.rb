class RemoveNonUniqueIndexFromBeginningStocks < ActiveRecord::Migration[5.0]
  def change
    ...
    add_index :beginning_stock_products, [:beginning_stock_month_id, :product_id], :unique => true, name: "index_beginning_stock_products_on_bsm_id_and_product_id" if ActiveRecord::Base.connection.table_exists? 'beginning_stock_months'
    add_index :beginning_stock_product_details, [:beginning_stock_product_id, :color_id, :size_id], :unique => true, name: "index_beginning_stock_product_details_on_bsp_color_size_id" if ActiveRecord::Base.connection.table_exists? 'beginning_stock_product_details'
