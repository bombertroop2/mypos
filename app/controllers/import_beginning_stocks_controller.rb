class ImportBeginningStocksController < ApplicationController
  authorize_resource class: BeginningStockProduct
    ...
    def create
      ...
      import_date = params[:import_date].to_date      
      error_message = ""
      error_message = "Import date must be before or equal to today" if import_date > Date.current      
      ...
      if error_message.present?
        ...
        if valid
          render js: "bootbox.alert({message: \"Beginning stocks were successfully imported\",size: 'small'});"
        else
          render js: "bootbox.alert({message: \"#{error_message}\",size: 'small'});"
        end
      end
