...
class SalesReturnsController < ApplicationController
  ...
  def index
    ...
    sales_returns_scope = if current_user.has_non_spg_role?
      SalesReturn.joins(returned_receipt: :cashier_opening).select(:id, :total_return, :total_return_quantity, :document_number, :created_at).select("sales.transaction_number")
    else
      SalesReturn.joins(returned_receipt: :cashier_opening).select(:id, :total_return, :total_return_quantity, :document_number, :created_at).select("sales.transaction_number").where(["cashier_openings.warehouse_id = ?", current_user.sales_promotion_girl.warehouse_id])
    end
    ...
    sales_returns_scope = sales_returns_scope.where(["cashier_openings.warehouse_id = ?", params[:filter_warehouse_id]]) if params[:filter_warehouse_id].present?
  ...
  def create
    ...
    if session["member"].present? && session["member"]["discount"].present? && session["member"]["discount"] > 0
      @total = @total - @total * (session["member"]["discount"].to_f / 100)
      ...
      begin
        ...
        unless @sales_return.save
          if @sales_return.errors[:base].present?
            if @sales_return.errors[:base].join("<br/>").include?("Please open the cashier first")
              render js: "bootbox.alert({message: \"#{@sales_return.errors[:base].join("<br/>")}\",size: 'small',callback: function () {window.location = '#{new_cashier_opening_url}';}});"
            else
  ...
  def search_receipt
    @sale_products = SaleProduct.
      ...
      select(:id, :total, :sale_id, "sizes.size AS product_size", "common_fields.name AS product_color", "products.code AS product_code", "sales.transaction_time", "sales.gift_event_id", "sales.member_id", "sales_returns.sale_id AS returned_sale_id", "sales.member_discount AS sale_member_discount", "sales.member_product_event AS sale_member_product_event", "sales.member_discount AS sale_member_discount").
    ...
    if @sale_products.present?
      ...
      session["member"] = {}
      session["member"]["id"] = sale_product.member_id
      session["member"]["discount"] = sale_product.sale_member_discount
      session["member"]["product_event"] = sale_product.sale_member_product_event
    end      
  end
    ...
    if @returned_product.product_event.eql?("Buy 1 Get 1 Free")
      ...
      if @replacement_product.present?
        ...
        if @returned_product.price > @replacement_product.price && @returned_product.barcode != @replacement_product.barcode
          ...
          @store_event = nil
          if session["member"].present? && session["member"]["product_event"] != true
          else
          ...
          if @store_event.blank?
            ...
            replacement_product_price = if session["member"].present? && session["member"]["discount"].present? && session["member"]["discount"] > 0
              @replacement_product.price - @replacement_product.price * (session["member"]["discount"].to_f / 100)
            else
              @replacement_product.price
            end
            if @returned_product.total > replacement_product_price && @returned_product.barcode != @replacement_product.barcode
            ...
            if @store_event.event_type.eql?("Buy 1 Get 1 Free")
              ...
              subtotal = if @store_event.event_type.eql?("Discount(%)") && @store_event.first_plus_discount.present? && @store_event.second_plus_discount.present?
                ...
                second_discounted_subtotal = first_discounted_subtotal - first_discounted_subtotal * @store_event.second_plus_discount / 100
                if session["member"].present? && session["member"]["discount"].present? && session["member"]["discount"] > 0
                  second_discounted_subtotal - second_discounted_subtotal * (session["member"]["discount"].to_f / 100)
                else
                  second_discounted_subtotal
                end
              elsif @store_event.event_type.eql?("Discount(%)") && @store_event.first_plus_discount.present?
                discounted_subtotal = @replacement_product.price - @replacement_product.price * @store_event.first_plus_discount / 100
                if session["member"].present? && session["member"]["discount"].present? && session["member"]["discount"] > 0
                  discounted_subtotal - discounted_subtotal * (session["member"]["discount"].to_f / 100)
                else
                  discounted_subtotal
                end
              elsif @store_event.event_type.eql?("Special Price") && @store_event.special_price.present?
                sp = @store_event.special_price
                if session["member"].present? && session["member"]["discount"].present? && session["member"]["discount"] > 0
                  sp - sp * (session["member"]["discount"].to_f / 100)
                else
                  sp
                end
              elsif @store_event.event_type.eql?("Discount(Rp)") && @store_event.cash_discount.present?
                discounted_subtotal = @replacement_product.price - @store_event.cash_discount
                if session["member"].present? && session["member"]["discount"].present? && session["member"]["discount"] > 0
                  discounted_subtotal - discounted_subtotal * (session["member"]["discount"].to_f / 100)
                else
                  discounted_subtotal
  ...
  def sales_return_params
    params.require(:sales_return).permit(:sale_id, :total_return, :total_return_quantity, :attr_receipt_number, :attr_cashier_id, :attr_spg_id, :attr_sale_member_product_event, :attr_sale_member_discount,
      sale_attributes: [:payment_method, :cash, :bank_id, :card_number, :trace_number, :pay, :cashier_id, :attr_return_sale_products, :total, :warehouse_id, :member_id, :gross_profit, :member_product_event, :member_discount,
        sale_products_attributes: [:product_barcode_id, :event_id, :returned_product_id, :sales_promotion_girl_id, :effective_price, :price_list_id, :event_type, :free_product_id, :first_plus_discount, :second_plus_discount, :cash_discount, :quantity, :attr_returned_sale_id, :attr_returning_sale, :cost_list_id, :attr_effective_cost, :attr_member_discount]],
  ...
  def add_additional_params_to_sales_return
    ...
    if session["member"].present? 
      hash_params = {cashier_id: current_user.id, attr_return_sale_products: true, warehouse_id: warehouse_id, member_id: session["member"]["id"], member_product_event: session["member"]["product_event"], member_discount: session["member"]["discount"]}
      hash_params_sales_return = {attr_cashier_id: current_user.id, attr_spg_id: current_user.sales_promotion_girl_id, attr_sale_member_product_event: session["member"]["product_event"], attr_sale_member_discount: session["member"]["discount"]}
    else
      hash_params = {cashier_id: current_user.id, attr_return_sale_products: true, warehouse_id: warehouse_id, member_id: nil, member_product_event: nil, member_discount: 0}
      hash_params_sales_return = {attr_cashier_id: current_user.id, attr_spg_id: current_user.sales_promotion_girl_id, attr_sale_member_product_event: nil, attr_sale_member_discount: 0}
    end
    params[:sales_return].merge! hash_params_sales_return
    ...
    params[:sales_return][:sale_attributes][:sale_products_attributes].each do |key, value|
      attr_member_discount = if session["member"].present?
        session["member"]["discount"]
      end
      params[:sales_return][:sale_attributes][:sale_products_attributes][key].merge! attr_returned_sale_id: params[:sales_return][:sale_id], attr_returning_sale: true, attr_member_discount: attr_member_discount
