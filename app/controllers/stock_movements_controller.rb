    ...
    if params[:year].present?
      ...
      unless params[:product_id].present?
        ...
        @previous_stock_movement_transactions = if @warehouse.warehouse_type.eql?("central")
          StockMovementTransaction.joins(stock_movement_product_detail: [:color, :size, stock_movement_product: [:stock_movement_warehouse, product: :brand]]).select("SUM(purchase_order_quantity_received) AS total_po_quantity_received, SUM(purchase_return_quantity_returned) AS total_pr_quantity_returned, SUM(delivery_order_quantity_delivered) AS total_do_quantity_delivered, SUM(stock_return_quantity_received) AS total_stock_return_quantity_received, products.code AS product_code, common_fields.code AS color_code, common_fields.name AS color_name, size AS product_size, brands_products.name AS product_name, stock_movement_product_detail_id").where(["warehouse_id = ? AND transaction_date <= ?", @warehouse.id, @selected_date.prev_month.end_of_month]).group(:stock_movement_product_detail_id).order(:transaction_date)
        else
          StockMovementTransaction.joins(stock_movement_product_detail: [:color, :size, stock_movement_product: [:stock_movement_warehouse, product: :brand]]).select("SUM(delivery_order_quantity_received) AS total_do_quantity_received, SUM(stock_return_quantity_returned) AS total_stock_return_quantity_returned, SUM(stock_transfer_quantity_received) AS total_stock_transfer_quantity_received, SUM(stock_transfer_quantity_delivered) AS total_stock_transfer_quantity_delivered, products.code AS product_code, common_fields.code AS color_code, common_fields.name AS color_name, size AS product_size, brands_products.name AS product_name, stock_movement_product_detail_id").where(["warehouse_id = ? AND transaction_date <= ?", @warehouse.id, @selected_date.prev_month.end_of_month]).group(:stock_movement_product_detail_id).order(:transaction_date)
        end
      else
        @stock_movement_transactions = if @warehouse.warehouse_type.eql?("central")
          StockMovementTransaction.joins(stock_movement_product_detail: [:color, :size, stock_movement_product: [stock_movement_warehouse: [stock_movement_month: :stock_movement], product: :brand]]).select("SUM(purchase_order_quantity_received) AS total_po_quantity_received, SUM(purchase_return_quantity_returned) AS total_pr_quantity_returned, SUM(delivery_order_quantity_delivered) AS total_do_quantity_delivered, SUM(stock_return_quantity_received) AS total_stock_return_quantity_received, products.code AS product_code, common_fields.code AS color_code, common_fields.name AS color_name, size AS product_size, brands_products.name AS product_name, stock_movement_product_detail_id").where(["warehouse_id = ? AND month = ? AND year = ? AND transaction_date BETWEEN ? AND ? AND products.id = ?", @warehouse.id, @selected_date.month, @selected_date.year, @selected_date.beginning_of_month, @selected_date.end_of_month, params[:product_id]]).group(:stock_movement_product_detail_id).order(:transaction_date)
        else
          StockMovementTransaction.joins(stock_movement_product_detail: [:color, :size, stock_movement_product: [stock_movement_warehouse: [stock_movement_month: :stock_movement], product: :brand]]).select("SUM(delivery_order_quantity_received) AS total_do_quantity_received, SUM(stock_return_quantity_returned) AS total_stock_return_quantity_returned, SUM(stock_transfer_quantity_received) AS total_stock_transfer_quantity_received, SUM(stock_transfer_quantity_delivered) AS total_stock_transfer_quantity_delivered, products.code AS product_code, common_fields.code AS color_code, common_fields.name AS color_name, size AS product_size, brands_products.name AS product_name, stock_movement_product_detail_id").where(["warehouse_id = ? AND month = ? AND year = ? AND transaction_date BETWEEN ? AND ? AND products.id = ?", @warehouse.id, @selected_date.month, @selected_date.year, @selected_date.beginning_of_month, @selected_date.end_of_month, params[:product_id]]).group(:stock_movement_product_detail_id).order(:transaction_date)
        end
        @previous_stock_movement_transactions = if @warehouse.warehouse_type.eql?("central")
          StockMovementTransaction.joins(stock_movement_product_detail: [:color, :size, stock_movement_product: [:stock_movement_warehouse, product: :brand]]).select("SUM(purchase_order_quantity_received) AS total_po_quantity_received, SUM(purchase_return_quantity_returned) AS total_pr_quantity_returned, SUM(delivery_order_quantity_delivered) AS total_do_quantity_delivered, SUM(stock_return_quantity_received) AS total_stock_return_quantity_received, products.code AS product_code, common_fields.code AS color_code, common_fields.name AS color_name, size AS product_size, brands_products.name AS product_name, stock_movement_product_detail_id").where(["warehouse_id = ? AND transaction_date <= ? AND products.id = ?", @warehouse.id, @selected_date.prev_month.end_of_month, params[:product_id]]).group(:stock_movement_product_detail_id).order(:transaction_date)
        else
          StockMovementTransaction.joins(stock_movement_product_detail: [:color, :size, stock_movement_product: [:stock_movement_warehouse, product: :brand]]).select("SUM(delivery_order_quantity_received) AS total_do_quantity_received, SUM(stock_return_quantity_returned) AS total_stock_return_quantity_returned, SUM(stock_transfer_quantity_received) AS total_stock_transfer_quantity_received, SUM(stock_transfer_quantity_delivered) AS total_stock_transfer_quantity_delivered, products.code AS product_code, common_fields.code AS color_code, common_fields.name AS color_name, size AS product_size, brands_products.name AS product_name, stock_movement_product_detail_id").where(["warehouse_id = ? AND transaction_date <= ? AND products.id = ?", @warehouse.id, @selected_date.prev_month.end_of_month, params[:product_id]]).group(:stock_movement_product_detail_id).order(:transaction_date)
        end
      end
