    ...
    if params[:year].present?
      ...
      unless params[:product_code].present?
        @stock_movement_transactions = if @warehouse.warehouse_type.eql?("central")
          StockMovementTransaction.joins(stock_movement_product_detail: [stock_movement_product: [:product, stock_movement_warehouse: [stock_movement_month: :stock_movement]]]).select("SUM(purchase_order_quantity_received) AS total_po_quantity_received, SUM(purchase_return_quantity_returned) AS total_pr_quantity_returned, SUM(delivery_order_quantity_delivered) AS total_do_quantity_delivered, SUM(stock_return_quantity_received) AS total_stock_return_quantity_received, stock_movement_product_detail_id").where(["warehouse_id = ? AND month = ? AND year = ? AND transaction_date BETWEEN ? AND ?", @warehouse.id, @selected_date.month, @selected_date.year, @selected_date.beginning_of_month, @selected_date.end_of_month]).group(:stock_movement_product_detail_id, :"products.code").order("products.code")
        else
          StockMovementTransaction.joins(stock_movement_product_detail: [stock_movement_product: [:product, stock_movement_warehouse: [stock_movement_month: :stock_movement]]]).select("SUM(delivery_order_quantity_received) AS total_do_quantity_received, SUM(stock_return_quantity_returned) AS total_stock_return_quantity_returned, SUM(stock_transfer_quantity_received) AS total_stock_transfer_quantity_received, SUM(stock_transfer_quantity_delivered) AS total_stock_transfer_quantity_delivered, stock_movement_product_detail_id").where(["warehouse_id = ? AND month = ? AND year = ? AND transaction_date BETWEEN ? AND ?", @warehouse.id, @selected_date.month, @selected_date.year, @selected_date.beginning_of_month, @selected_date.end_of_month]).group(:stock_movement_product_detail_id, :"products.code").order("products.code")
        end
        ...
        @stock_movement_transactions = if @warehouse.warehouse_type.eql?("central")
          StockMovementTransaction.joins(stock_movement_product_detail: [stock_movement_product: [:product, stock_movement_warehouse: [stock_movement_month: :stock_movement]]]).select("SUM(purchase_order_quantity_received) AS total_po_quantity_received, SUM(purchase_return_quantity_returned) AS total_pr_quantity_returned, SUM(delivery_order_quantity_delivered) AS total_do_quantity_delivered, SUM(stock_return_quantity_received) AS total_stock_return_quantity_received, stock_movement_product_detail_id").where(["warehouse_id = ? AND month = ? AND year = ? AND transaction_date BETWEEN ? AND ? AND code #{like_command} ?", @warehouse.id, @selected_date.month, @selected_date.year, @selected_date.beginning_of_month, @selected_date.end_of_month, "%"+params[:product_code]+"%"]).group(:stock_movement_product_detail_id, :"products.code").order("products.code")
        else
          StockMovementTransaction.joins(stock_movement_product_detail: [stock_movement_product: [:product, stock_movement_warehouse: [stock_movement_month: :stock_movement]]]).select("SUM(delivery_order_quantity_received) AS total_do_quantity_received, SUM(stock_return_quantity_returned) AS total_stock_return_quantity_returned, SUM(stock_transfer_quantity_received) AS total_stock_transfer_quantity_received, SUM(stock_transfer_quantity_delivered) AS total_stock_transfer_quantity_delivered, stock_movement_product_detail_id").where(["warehouse_id = ? AND month = ? AND year = ? AND transaction_date BETWEEN ? AND ? AND code #{like_command} ?", @warehouse.id, @selected_date.month, @selected_date.year, @selected_date.beginning_of_month, @selected_date.end_of_month, "%"+params[:product_code]+"%"]).group(:stock_movement_product_detail_id, :"products.code").order("products.code")
        end
        @previous_stock_movement_transactions = if @warehouse.warehouse_type.eql?("central")
