    ...
    respond_to do |format|
      if params[:date].present? && params[:counter].present?
        ...
        @shipment_products = ShipmentProduct.
          select("order_booking_products.product_id, products.code AS product_code, common_fields.name AS product_name, SUM(shipment_products.quantity) AS qty_received_per_article").
          joins(order_booking_product: [product: :brand], shipment: :order_booking).
          where(["shipments.received_date = ? AND order_bookings.destination_warehouse_id = ?", @received_date, params[:counter]]).
          group("order_booking_products.product_id, products.code, common_fields.name")
        ...
        @consignment_sale_products = ConsignmentSaleProduct.
          ...
          where(:"product_colors.product_id" => @shipment_products.map(&:product_id)).
          group("product_colors.product_id")
        format.js
      else
        format.html
      end
