class Shipment < ApplicationRecord
  ...
  attr_accessor :order_booking_number, :receiving_inventory, :attr_change_receive_date, :attr_user_id
  ...
  validates :delivery_date, date: {after_or_equal_to: proc {|shpmnt| shpmnt.order_booking.created_at.to_date}, message: 'must be after or equal to creation date of order booking' }, if: proc {|shpmnt| shpmnt.delivery_date.present? && shpmnt.order_booking_id.present? && !shpmnt.attr_change_receive_date}
    validates :delivery_date, date: {after_or_equal_to: proc { Date.current }, message: 'must be after or equal to today' }, if: proc {|shpmnt| shpmnt.delivery_date.present? && shpmnt.delivery_date_changed? && !shpmnt.receiving_inventory && !shpmnt.attr_change_receive_date && !shpmnt.order_booking.destination_warehouse.warehouse_type.eql?("direct_sales")}
      validates :delivery_date, date: {before_or_equal_to: proc { Date.current }, message: 'must be before or equal to today' }, if: proc {|shpmnt| shpmnt.delivery_date.present? && shpmnt.delivery_date_changed? && shpmnt.order_booking.destination_warehouse.warehouse_type.eql?("direct_sales")}
                  ...
                  validate :new_received_date_valid, if: proc {|shpmnt| shpmnt.received_date.present? && shpmnt.attr_change_receive_date}
                    validate :shipment_receivable, if: proc{|shpmnt| shpmnt.receiving_inventory}
                      validate :minimum_quantity, if: proc{|shpmnt| !shpmnt.receiving_inventory && !shpmnt.attr_change_receive_date}
                        validate :transaction_open, if: proc{|shipment| shipment.receiving_inventory || shipment.attr_change_receive_date || (shipment.delivery_date.present? && shipment.delivery_date_changed? && shipment.order_booking.destination_warehouse.warehouse_type.eql?("direct_sales"))}
                                ...
                                before_update :set_is_receive_date_changed, :change_listing_stock_transaction_date, if: proc{|shipment| shipment.attr_change_receive_date}
                                  after_update :empty_in_transit_warehouse, :load_goods_to_destination_warehouse, if: proc{|shpmnt| shpmnt.receiving_inventory}
                                    ...
                                    def transaction_after_beginning_stock_added
                                      shipment_product_items.joins(order_booking_product_item: [order_booking_product: :order_booking]).select("order_bookings.destination_warehouse_id", "order_booking_products.product_id", "order_booking_product_items.size_id", "order_booking_product_items.color_id").each do |spi|
                                        listing_stock_transaction = ListingStockTransaction.select(:transaction_date).joins(listing_stock_product_detail: :listing_stock).where(transaction_type: "BS", :"listing_stock_product_details.color_id" => spi.color_id, :"listing_stock_product_details.size_id" => spi.size_id, :"listing_stocks.warehouse_id" => spi.destination_warehouse_id, :"listing_stocks.product_id" => spi.product_id).first
                                        if listing_stock_transaction.present? && listing_stock_transaction.transaction_date > received_date
                                          errors.add(:base, "Sorry, you can't perform transaction on #{received_date.strftime("%d/%m/%Y")}")
                                          break
                                        end
                                      end
                                    end
                                      ...
                                      if order_booking_id.present?
                                        ...
                                        if @order_booking.present?
                                          ...
                                          if origin_warehouse.blank?
                                            ...
                                            @destination_warehouse = Warehouse.select(:id, :code, :warehouse_type).where(id: @order_booking.destination_warehouse_id, is_active: true).first
                                    ...
                                    def transaction_open
                                      if !receiving_inventory && !attr_change_receive_date
                                        ...
                                        if receiving_inventory && attr_change_receive_date
                                          ...
                                          errors.add(:base, "Sorry, you can't perform this transaction") if FiscalYear.joins(:fiscal_months).where(year: delivery_date.year).where("fiscal_months.month = '#{Date::MONTHNAMES[delivery_date.month]}' AND fiscal_months.status = 'Close'").select("1 AS one").present?
                                        end
                                      end
                                    end
                                    ...
                                    def editable
                                      errors.add(:base, "The record cannot be edited") if !is_document_printed_changed?
                                    end
                                    ...
                                    def deletable
                                      user_roles = User.select(:id).find(attr_user_id).roles.pluck :name
                                      if received_date.present? || is_document_printed
                                        if (user_roles.include?("administrator") || user_roles.include?("superadmin")) && order_booking.destination_warehouse.warehouse_type.eql?("direct_sales")
                                        else
