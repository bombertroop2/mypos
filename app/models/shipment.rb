class Shipment < ApplicationRecord
  ...
  before_validation :remove_courier_cost, on: :create
  
  validates :delivery_date, :courier_id, :order_booking_id, presence: true
  validates :courier_cost, presence: true, on: :create, if: proc{@courier_status.eql?("External")}
                          ...
                          validate :receive_date_not_changed, if: proc{|shipment| shipment.attr_change_receive_date}
                            validate :transaction_after_beginning_stock_added, if: proc{|shpmnt| shpmnt.received_date_changed?}
                              validate :received_date_changeable, if: proc{|shpmnt| shpmnt.received_date.present? && shpmnt.attr_change_receive_date}
                                validates :courier_cost, numericality: {greater_than_or_equal_to: 1}, on: :create, if: proc { |s| s.courier_cost.present? && s.courier_cost.is_a?(Numeric) && @courier_status.eql?("External") }
                                  ...
                                  before_update :set_is_receive_date_changed, :change_listing_stock_transaction_date, if: proc{|shipment| shipment.attr_change_receive_date}
                                    after_update :empty_in_transit_warehouse, :load_goods_to_destination_warehouse, if: proc{|shpmnt| shpmnt.receiving_inventory}
                                      ...
                                      def remove_courier_cost
                                        self.courier_cost = nil if courier_id.present? && (@courier_status = Courier.select(:status).find(courier_id).status).eql?("Internal")
                                      end
