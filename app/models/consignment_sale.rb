class ConsignmentSale < ApplicationRecord
  attr_accessor :attr_warehouse_code, :attr_user_ids, :attr_delete_by_admin,
    :attr_create_by_admin, :attr_spg_warehouse_id, :attr_am_warehouse_ids, :attr_supervisor_id,
    :attr_create_by_am, :attr_delete_products
  audited on: [:create, :update]
  has_many :consignment_sale_products, dependent: :destroy
  has_many :listing_stock_transactions, as: :transactionable
  belongs_to :warehouse
  
  validate :product_is_exist, on: :create, unless: proc{|cs| cs.no_sale}
    validate :transaction_date_is_valid, :transaction_date_not_blank, on: :create
    validate :warehouse_not_blank, :warehouse_is_counter, if: proc{|cs| (cs.attr_create_by_admin || cs.attr_create_by_am) && !cs.attr_delete_products}
      ...
      validate :transaction_after_beginning_stock_added, if: proc{|sale| !sale.attr_delete_products && Company.where(import_beginning_stock: true).select("1 AS one").present?}
        validate :record_not_updated, :record_belongs_to_me, if: proc{|cs| cs.attr_delete_products}, on: :update
          ...
          before_create :generate_number, :delete_counter_event_id, :sale_not_created
                ...
                after_update :delete_listing_stock, :delete_stock_movement, if: proc{|cs| cs.approved_was && !cs.approved && !cs.attr_delete_products}
                  ...
                  def sale_not_created
                    if no_sale
                      ...
                      warehouse_id = if self.warehouse_id.present?
                        ...
                        Warehouse.select(:id).where(["code = ?", attr_warehouse_code]).first.id
                      end
                  ...
                  def record_belongs_to_me
                    if attr_user_ids.present?
                      if ConsignmentSale.select("1 AS one").joins(:audits).where(id: id, :"audits.user_id" => attr_user_ids, :"audits.action" => "create").blank?
                        errors.add(:base, "The record cannot be edited")
                      end
                    elsif attr_spg_warehouse_id.present? 
