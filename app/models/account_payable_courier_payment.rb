class AccountPayableCourierPayment < ApplicationRecord
  attr_accessor :attr_courier_code_and_name, :attr_company_bank_id
  audited on: :create

  belongs_to :courier
  belongs_to :company_bank_account_number
  has_many :account_payable_courier_payment_invoices, dependent: :destroy
  
  accepts_nested_attributes_for :account_payable_courier_payment_invoices, allow_destroy: true

  before_validation :strip_string_values

  validates :payment_date, :payment_method, :courier_id, presence: true
  validates :attr_company_bank_id, :company_bank_account_number_id, presence: true, if: proc{|app| app.payment_method.eql?("Transfer")}
    validates :payment_date, date: {before_or_equal_to: proc { Date.current }, message: 'must be before or equal to today' }, if: proc {|app| app.payment_date.present?}
      ...
      validate :transaction_open, :payment_method_available, :courier_available, :payment_date_after_or_equal_to_latest_payment_date
              ...
              validates :giro_number, uniqueness: true, if: proc {|app| app.payment_method.eql?("Giro")}
                ...
                def payment_date_after_or_equal_to_latest_payment_date
                  if payment_date.present?
                    account_payable_courier_payment_invoices.each do |account_payable_courier_payment_invoice|
                      latest_account_payable_courier_payment_invoice = AccountPayableCourierPaymentInvoice.select("account_payable_courier_payments.payment_date").joins(:account_payable_courier_payment).where(account_payable_courier_id: account_payable_courier_payment_invoice.account_payable_courier_id).order("account_payable_courier_payments.payment_date DESC").first
                      if latest_account_payable_courier_payment_invoice.present? && payment_date < latest_account_payable_courier_payment_invoice.payment_date
                        errors.add(:payment_date, "must be after or equal to #{latest_account_payable_courier_payment_invoice.payment_date.strftime("%d/%m/%Y")}")
                        break
                      end
                    end
                  end
                ...
                def check_min_invoice_per_payment
                  errors.add(:base, "Payment must have at least one AP invoice") if account_payable_courier_payment_invoices.blank?
                end
                ...
                def courier_available
                  errors.add(:courier_id, "does not exist!") if (@courier = Courier.select(:code).where(id: courier_id, status: "External").first).blank?
                end
                ...
                def generate_number
                  ...
                  existed_numbers = AccountPayableCourierPayment.where("number LIKE 'PY#{@courier.code}#{current_month}#{current_year}%'").select(:number).order(:number)
                  if existed_numbers.blank?
                    new_number = "PY#{@courier.code}#{current_month}#{current_year}0001"
                  else
                    if existed_numbers.length == 1
                      seq_number = existed_numbers[0].number.split("PY#{@courier.code}#{current_month}#{current_year}").last
                      if seq_number.to_i > 1
                        new_number = "PY#{@courier.code}#{current_month}#{current_year}0001"
                      else
                        new_number = "PY#{@courier.code}#{current_month}#{current_year}#{seq_number.succ}"
                      end
                    else
                      last_seq_number = ""
                      existed_numbers.each_with_index do |existed_number, index|
                        seq_number = existed_number.number.split("PY#{@courier.code}#{current_month}#{current_year}").last
                        if seq_number.to_i > 1 && index == 0
                          ...
                          new_number = "PY#{@courier.code}#{current_month}#{current_year}#{last_seq_number.succ}"
                          break
