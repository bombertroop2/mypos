                          ...
                          before_update :calculate_order_value, if: proc {|po| !po.receiving_po && !po.closing_po && !po.edit_document}
                            ...
                            before_update :calculate_net_amount, if: proc{|po| po.edit_document}
                              ...
                              after_update :update_product_cost, if: proc{|po| po.purchase_order_date_changed?}
                                ...
                                def get_gross_price
                                  costs = self.received_purchase_orders.joins(received_purchase_order_products: [:received_purchase_order_items, purchase_order_product: [[product: :brand], :cost_list]]).select("name, cost, purchase_order_products.id, products.code AS product_code, received_purchase_order_items.quantity")
                                  gross_price = costs.collect{|x| x.cost.to_i * x.quantity}.sum
                                  return gross_price
                                end
