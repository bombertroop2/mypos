...
#  has_many :products, through: :purchase_order_products, group: "products.id"
...
#
  ...
  after_save :calculate_order_value

  before_destroy :prevent_destroy_if_article_received

  validates :number, presence: true, uniqueness: true, unless: proc { |po| po.receiving_po.eql?("true") }
  validates :supplier_id, :means_of_payment, presence: true, unless: proc { |po| po.receiving_po.eql?("true") }
  ...
  validates :request_delivery_date, presence: true, unless: proc { |po| po.receiving_po.eql?("true") }
