                ...
                validates :cash, numericality: {greater_than_or_equal_to: :sale_total}, if: proc { |sale| sale.cash.is_a?(Numeric) && sale.cash.present? && sale.payment_method.eql?("Cash") }
                  validates :bank_id, :trace_number, :card_number, :pay, presence: true, if: proc{|sale| sale.payment_method.eql?("Card")}
                    validate :bank_available, if: proc{|sale| sale.payment_method.eql?("Card") && sale.bank_id.present?}
                      validates :pay, numericality: {equal_to: :sale_total, message: "must be equal to %{count}"}, if: proc { |sale| sale.pay.present? && sale.payment_method.eql?("Card") }
                            ...
                            validates :gift_event_product_id, presence: true, if: proc { |sp| sp.gift_event_gift_type.eql?("Product") && sp.gift_event_id.present? }
                              ...
                              before_create :set_change, if: proc{|sale| sale.payment_method.eql?("Cash")}
                                before_create :strip_card_and_trace_number, if: proc{|sale| sale.payment_method.eql?("Card")}
                                  after_create :update_stock, if: proc {|sale| sale.gift_event_product_id.present? && sale.gift_event_id.present? && sale.gift_event_gift_type.eql?("Product") }
