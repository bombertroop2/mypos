class Sale < ApplicationRecord
  ...
  validates :member_discount, presence: true, if: proc {|sale| sale.member_id.present? && !sale.attr_return_sale_products}
  validate :member_discount_valid, if: proc {|sale| sale.member_discount.present? && !sale.attr_return_sale_products}
  ...
  validates :member_id, presence: {message: "Please select member first"}, if: proc {|sale| sale.payment_method.eql?("Card") && !sale.attr_return_sale_products && @bank.present? && @bank.code.eql?("MEMO")}
  validate :memo_not_allowed, if: proc {|sale| sale.payment_method.eql?("Card") && sale.attr_return_sale_products && @bank.present? && @bank.code.eql?("MEMO")}
  validate :card_number_not_duplicate, if: proc {|sale| sale.card_number.present? && sale.card_number_changed? && sale.payment_method.eql?("Card") && !sale.attr_return_sale_products && @bank.present? && @bank.code.eql?("MEMO")}
  ...
  before_create :set_trace_number_as_memo, if: proc {|sale| sale.payment_method.eql?("Card") && !sale.attr_return_sale_products && @bank.present? && @bank.code.eql?("MEMO")}
  ...
  before_create :set_member_product_event, if: proc{|sale| !sale.attr_return_sale_products && sale.member_id.present?}
  ...
  def card_number_not_duplicate
    errors.add(:card_number, "has already been taken") if Sale.select("1 AS one").where(["LOWER(card_number) = ? AND trace_number = 'MEMO'", card_number.downcase]).present?
  end
                                                                
  def set_trace_number_as_memo
    self.trace_number = "MEMO"
  end
                                                            
  def memo_not_allowed
    errors.add(:base, "Payment method MEMO is not allowed!")
  end
                                                      
  def set_member_product_event
    if @member.present?
      self.member_product_event = @member.member_product_event
    end
  end
  ...
  def bank_available
    if (@bank = Bank.select(:code).where(id: bank_id).first).blank?
  ...
  def member_available
    errors.add(:base, "Members not found") if (@member = Member.select(:discount, :member_product_event).where(id: member_id).first).blank?
  end

  def member_discount_valid
    if @member.present?
      errors.add(:base, "Member discount is not valid") if @member.discount != member_discount
    else
      errors.add(:base, "Member discount is not valid") if member_discount > 0
    end
  end
