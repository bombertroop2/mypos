class AccountPayable < ApplicationRecord
  ...
  include Accounting::TransactionCashOut
                ...
                validates :debt, numericality: true, if: proc { |ap| ap.debt.present? }
                  validates :debt, numericality: {greater_than_or_equal_to: 0}, if: proc { |ap| ap.debt.present? && ap.debt.is_a?(Numeric) }
                    validate :calculate_amount_paid_and_debt, if: proc{|ap| ap.amount_paid.present? && ap.amount_paid.is_a?(Numeric) && ap.debt.present? && ap.debt.is_a?(Numeric) && ap.amount_paid > 0}, on: :create
                      ...
                      validate :payment_date_after_or_equal_to_last_payment_date, if: proc {|ap| ap.payment_date.present?}, on: :create
                        ...
                        def payment_date_after_or_equal_to_last_payment_date
                          account_payable_purchases.each do |account_payable_purchase|
                            last_ap = if payment_for_dp
                              AccountPayable.select(:payment_date).joins(:account_payable_purchases).where("purchase_id = #{account_payable_purchase.purchase_id} AND purchase_type = 'DirectPurchase'").last
                            else
                              AccountPayable.select(:payment_date).joins(:account_payable_purchases).where("purchase_id = #{account_payable_purchase.purchase_id} AND purchase_type = 'PurchaseOrder'").last
                            end
                            if last_ap.present? && last_ap.payment_date > payment_date
                              errors.add(:payment_date, "must be after or equal to last payment date")
                              break
                            end
                          end
                        end
