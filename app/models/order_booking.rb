    ...
    validate :editable, on: :update, unless: proc{|ob| ob.picking_note}
      validate :pickable_note, if: proc{|ob| ob.picking_note}
        validate :transaction_open, if: proc{|ob| ob.plan_date.present? && !ob.picking_note}
          validate :customer_id_selected, if: proc{|ob| ob.destination_warehouse_id.present? && ob.destination_warehouse_id_changed?}
            validate :customer_available, if: proc{|ob| ob.destination_warehouse_id.present? && ob.destination_warehouse_id_changed?}
              ...
              def remove_customer_id
                self.customer_id = nil unless @dw.warehouse_type.eql?("direct_sales")
              end
                            
              def customer_available
                errors.add(:customer_id, "does not exist!") if customer_id.present? && Customer.select("1 AS one").where(id: customer_id).blank? && @dw.warehouse_type.eql?("direct_sales")
              end
            
              def customer_id_selected
              ...
              def destination_warehouse_available
                errors.add(:destination_warehouse_id, "does not exist!") if (new_record? || (destination_warehouse_id_changed? && persisted?)) && destination_warehouse_id.present? && (@dw = Warehouse.not_central.not_in_transit.actived.where(id: destination_warehouse_id).select(:warehouse_type).first).blank?
              end
