      ...
      validate :pickable_note, if: proc{|ob| ob.picking_note}
        validate :transaction_open, if: proc{|ob| ob.plan_date.present? && !ob.picking_note}
          ...
          def destination_warehouse_available
            errors.add(:destination_warehouse_id, "does not exist!") if (new_record? || (destination_warehouse_id_changed? && persisted?)) && destination_warehouse_id.present? && Warehouse.not_central.not_in_transit.not_direct_sales.actived.where(id: destination_warehouse_id).select("1 AS one").blank?
          end
