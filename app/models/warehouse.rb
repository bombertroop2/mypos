class Warehouse < ApplicationRecord
  ...
  validates :province_id, :city_id, presence: true, if: proc{|w| !w.warehouse_type.eql?("in_transit") && !w.warehouse_type.eql?("direct_sales")}
      ...
      validates :sku, :counter_type, presence: true, if: proc{|warehouse| warehouse.warehouse_type.present? && warehouse.warehouse_type.include?("ctr")}
        validates :code, length: {maximum: 9}, if: Proc.new {|warehouse| warehouse.code.present?}
          validate :code_not_changed, :is_area_manager_valid_to_supervise_this_warehouse?,
            ...
            :in_transit_present, :counter_type_not_changed, :direct_sales_present, :city_available, :city_id_not_changed
          validate :counter_type_available, if: proc{|wr| wr.warehouse_type.present? && wr.warehouse_type.include?("ctr")}
            validates :estimated_delivery_time, presence: true, if: proc{|w| w.warehouse_type.eql?("showroom") || w.warehouse_type.include?("ctr")}
              validates :estimated_delivery_time, numericality: {greater_than_or_equal_to: 1, only_integer: true}, if: proc {|w| w.estimated_delivery_time.present? && w.estimated_delivery_time_changed? && (w.warehouse_type.eql?("showroom") || w.warehouse_type.include?("ctr"))}

                before_validation :delete_non_intransit_attributes, if: proc{|warehouse| warehouse.warehouse_type.eql?("in_transit")}
                  before_validation :delete_non_direct_sales_attributes, if: proc{|warehouse| warehouse.warehouse_type.eql?("direct_sales")}
                    ...
                    before_validation :delete_sku_value, :delete_counter_type_value, if: proc{|warehouse| warehouse.warehouse_type.present? && !warehouse.warehouse_type.include?("ctr")}
                      before_save :empty_messages, unless: proc{|warehouse| warehouse.warehouse_type.eql?("showroom")}
                        ...
                        def city_id_not_changed
                          errors.add(:city_id, "change is not allowed!") if province_id.present? && city_id.present? && (province_id_changed? || city_id_changed?) && (province_id_was.present? || city_id_was.present?) && persisted? && destination_warehouse_order_booking_relation.present?
                        end
                        ...
                        def delete_non_direct_sales_attributes
                          self.province_id = nil
                          self.city_id = nil
                        end
