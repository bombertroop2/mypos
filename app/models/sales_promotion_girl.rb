class SalesPromotionGirl < ApplicationRecord
  ...
  def warehouse_available
    errors.add(:warehouse_id, "does not exist!") if warehouse_id.present? && Warehouse.where(id: warehouse_id, is_active: true).where("warehouse_type <> 'central' AND warehouse_type <> 'in_transit' AND warehouse_type <> 'direct_sales'").select("1 AS one").blank?
  end
