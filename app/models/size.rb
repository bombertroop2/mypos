class Size < ApplicationRecord
  ...
  validates :size_order, numericality: {only_integer: true}, if: proc { |dpd| dpd.size_order.present? }
    ...
    def size_not_changed
      errors.add(:size, "change is not allowed!") if size_changed? && persisted? && (order_booking_product_item_relation.present? || product_detail_relation.present? || stock_detail_relation.present? || stock_mutation_product_item_relation.present? || product_barcode_relation.present? || adjustment_product_detail_relation.present?)
    end
