class PurchaseReturn < ApplicationRecord
  attr_accessor :delivery_order_number, :direct_purchase_return, :attr_purchase_return_number, :attr_purchase_return_net_amount, :attr_transaction_number
  ...
  validate :purchase_order_is_returnable, if: proc {|pr| pr.purchase_order_id.present? && !pr.direct_purchase_return}
    validate :direct_purchase_is_returnable, if: proc {|pr| pr.direct_purchase_id.present? && pr.direct_purchase_return}
      validates :purchase_order_id, presence: true, unless: proc{|pr| pr.direct_purchase_return}
        validates :delivery_order_number, presence: true, if: proc{|pr| pr.direct_purchase_return && pr.attr_transaction_number.blank?}
          validates :attr_transaction_number, presence: true, if: proc{|pr| pr.direct_purchase_return && pr.delivery_order_number.blank?}
            validates :direct_purchase_id, presence: true, if: proc{|pr| pr.direct_purchase_return}
              validate :check_min_return_quantity, if: proc{|pr| pr.attr_transaction_number.present? || pr.delivery_order_number.present? || pr.purchase_order_id.present?}
                ...
                def set_attr_purchase_return_number
                  self.attr_purchase_return_number = if purchase_order_id.present?
                    ...
                    @purchase_doc.transaction_number
                  end              
                end
                ...
                def direct_purchase_is_returnable
                  if (@purchase_doc = DirectPurchase.where(["direct_purchases.id = ?", direct_purchase_id]).select(:id, :vat_type, :is_taxable_entrepreneur, :first_discount, :second_discount, :is_additional_disc_from_net, "received_purchase_orders.delivery_order_number", "received_purchase_orders.transaction_number").joins(:received_purchase_order).includes(direct_purchase_products: :cost_list).first).blank?
                    if delivery_order_number.present?
                      errors.add(:delivery_order_number, "does not exist!")
                    else
                      errors.add(:attr_transaction_number, "does not exist!")
                    end
                  end
                end
