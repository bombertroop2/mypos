class StockMutation < ApplicationRecord
  ...
  has_many :stock_mutation_product_items, through: :stock_mutation_products

  accepts_nested_attributes_for :stock_mutation_products, allow_destroy: true

  before_validation :remove_courier_cost
                      ...
                      validate :new_received_date_valid, if: proc {|sm| sm.received_date.present? && sm.attr_change_receive_date}
                        validate :receive_date_not_changed, if: proc{|sm| sm.attr_change_receive_date}
                          validate :article_sex_not_valid, if: proc{|sm| sm.destination_warehouse_id_changed? && sm.persisted?}
                            validate :transaction_after_beginning_stock_added, if: proc{|sm| sm.received_date_changed?}
                              validates :courier_cost, presence: true, if: proc{@courier_status.eql?("External")}
                                validates :courier_cost, numericality: {greater_than_or_equal_to: 1}, if: proc { |s| s.courier_cost.present? && s.courier_cost.is_a?(Numeric) && @courier_status.eql?("External") }
                                                ...
                                                after_update :update_stock, if: proc {|sm| sm.approving_mutation}
                                                  after_update :load_goods_to_destination_warehouse, if: proc{|sm| sm.receiving_inventory_to_store || sm.receiving_inventory_to_warehouse}
                                                    ...
                                                    def transaction_after_beginning_stock_added
                                                      stock_mutation_product_items.select("stock_mutation_products.product_id AS prdct_id", :size_id, :color_id).each do |smpi|
                                                        listing_stock_transaction = ListingStockTransaction.select(:transaction_date).joins(listing_stock_product_detail: :listing_stock).where(transaction_type: "BS", :"listing_stock_product_details.color_id" => smpi.color_id, :"listing_stock_product_details.size_id" => smpi.size_id, :"listing_stocks.warehouse_id" => destination_warehouse_id, :"listing_stocks.product_id" => smpi.prdct_id).first
